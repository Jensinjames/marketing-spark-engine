/*
  ...
  1. Security Enhancements
    ...
    - Drop and recreate all RLS policies with consistent naming
    - Add database-level validation constraints
    - Create audit logging system

  2. New Tables
    - `audit_logs` for tracking sensitive operations

  3. Functions
    - `audit_sensitive_operation` for logging admin actions
    - `update_updated_at_column` for automatic timestamp updates

  4. Triggers
    - Updated_at triggers for all tables with timestamp columns
*/
...
-- Add trigger to automatically reset credits when reset_at is reached
CREATE OR REPLACE FUNCTION public.reset_monthly_credits()
RETURNS TRIGGER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS reset_monthly_credits_trigger ON public.user_credits;
