      ...
      >
        {filters.map((filter, index) => (
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 ${
              activeFilter === filter
            ...
            aria-describedby={`filter-${filter.replace(/\s+/g, '-').toLowerCase()}-description`}
            tabIndex={activeFilter === filter ? 0 : -1}
          >
            {filter}
            <span 
              id={`filter-${filter.replace(/\s+/g, '-').toLowerCase()}-description`}
              className="sr-only"
            >
